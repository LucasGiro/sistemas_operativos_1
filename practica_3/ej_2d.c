...
{   
    ...
    while (!pthread_mutex_trylock(izq(i))) {
        ...
        usleep(random() % ESPERA);
        thread_mutex_lock(der(i));
    }
}

// item d- sí funciona, pero es lento, se podría hacer un contador de
// cantidad de pĺatos que se comieron en un determinado periodo de tiempo
// y compararlo con la solución de semaforos
